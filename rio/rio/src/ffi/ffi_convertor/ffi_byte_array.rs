...
impl FFIConvertor for Vec<u8> {
    ...
    unsafe fn from_ffi(ffi: Self::FFIType) -> Self {
        Vec::from_raw_parts(ffi.buffer, ffi.len, ffi.capacity)
    }

    unsafe fn free(ffi: Self::FFIType) {
        drop(Self::from_ffi(ffi))
    }
}
...
/// # Safety
#[no_mangle]
pub unsafe extern "C" fn free_array_buffer(array_buffer: FFIByteArray) {
    Vec::<u8>::free(array_buffer)
}
